...
Focused on quantum algorithms (VQE, QPE, QSVT), numerical simulation, and physics-driven modelling, with an emphasis on building **practical, package-first PyPI libraries**.
...
- Ground-state + excited-state solvers: **VQE, ADAPT-VQE, QSE, LR-VQE, VQD**
- **QPE** (noisy + noiseless) and **VarQITE / VarQRTE**
- Unified `common/` layer for Hamiltonians, molecule registry, geometry scans, plotting, and caching
...
- Variational Quantum Classifier (VQC), regression, QCNN, quantum kernels, metric learning
- Classical baselines for benchmarking
- Deterministic experiment pipelines with reproducible outputs
- CLI + API workflows; notebooks act as thin clients
...
- **Binary selection**: QUBO/Ising formulation solved with VQE and QAOA
- **Fractional allocation**: simplex-constrained ansatz
- Efficient frontier utilities, λ-sweeps, and structured outputs
- CLI + API workflows
...
- Explicit circuit simulation + distribution-based mode
...
Former Data Analyst with experience in:
...
LinkedIn:
https://www.linkedin.com/in/sid-richards-21374b30b/
...
GitHub Sponsors helps support continued work on:
